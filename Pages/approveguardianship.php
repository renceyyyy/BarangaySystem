...
if (isset($_GET['id'])) {
    ...
    $stmt = $connection->prepare("UPDATE guardianshiptbl SET RequestStatus = 'Approved' WHERE id = ?");
     ...
     if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            ...
            if (!empty($urlParams)) {
                ...
                $urlParams['panel'] = 'guardianshipPanel';
                $redirectUrl .= '?' . http_build_query($urlParams);
            } else {
                $redirectUrl .= '?panel=guardianshipPanel';
            }
